...
! function() {
    ...
    h("3", tinymce.util.Tools.resolve), g("1", ["3"], function(a) {
        ...
        return a.add("insertdatetime", function(a) {
                function c(b, c) {
                    function d(a, b) {
                        if (a = "" + a, a.length < b)
                    ...
                    return c = c || new Date, b = b.replace("%D", "%m/%d/%Y"), b = b.replace("%r", "%I:%M:%S %p"), b = b.replace("%Y", "" + c.getFullYear()), b = b.replace("%y", "" + c.getYear()), b = b.replace("%m", d(c.getMonth() + 1, 2)), b = b.replace("%d", d(c.getDate(), 2)), b = b.replace("%H", "" + d(c.getHours(), 2)), b = b.replace("%M", "" + d(c.getMinutes(), 2)), b = b.replace("%S", "" + d(c.getSeconds(), 2)), b = b.replace("%I", "" + ((c.getHours() + 11) % 12 + 1)), b = b.replace("%p", "" + (c.getHours() < 12 ? "AM" : "PM")), b = b.replace("%B", "" + a.translate(j[c.getMonth()])), b = b.replace("%b", "" + a.translate(i[c.getMonth()])), b = b.replace("%A", "" + a.translate(h[c.getDay()])), b = b.replace("%a", "" + a.translate(g[c.getDay()])), b = b.replace("%%", "%")
                }
                ...
                function d(b) {
                    ...
                    if (a.settings.insertdatetime_element) {
                        var e;
                        e = c(/%[HMSIp]/.test(b) ? "%Y-%m-%dT%H:%M" : "%Y-%m-%d"), d = '<time datetime="' + e + '">' + d + "</time>";
                        var f = a.dom.getParent(a.selection.getStart(), "time");
                        if (f) return void a.dom.setOuterHTML(f, d)
                    }
                    a.insertContent(d)
                }
                var e, f, g = "Dom Lun Mar Mie Jue Vie Sab Dom".split(" "),
                    h = "Domingo Lunes Martes Miércoles Jueves Viernes Sábado Domingo".split(" "),
                    i = "Ene Feb Mar Abr May Jun Jul Ago Sep Oct Nov Dic".split(" "),
                    j = "Enero Febrero Marzo Abril Mayo Junio Julio Agosto Septiembre Octubre Noviembre Diciembre".split(" "),
                    k = [];
